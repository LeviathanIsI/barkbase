...
describe('event ingestion idempotency', () => {
  ...
  it('deduplicates events by idempotency key', async () => {
    ...
    const runCountAfterFirst = await prisma.handlerRun.count({ where: { flowId: flow.recordId } });
    ...
    const runCountAfterSecond = await prisma.handlerRun.count({ where: { flowId: flow.recordId } });
    expect(runCountAfterSecond).toBe(runCountAfterFirst);

    const events = await prisma.handlerEvent.findMany({ where: { tenantId: tenant.recordId } });

...
const filePath = path.join(__dirname, '../lambdas/config-service/index.js');

// Read the file
let content = fs.readFileSync(filePath, 'utf8');

// Define the old handlers to replace
const oldHandlers = `async function getPackages(event, tenantId) {
    const pool = getPool();
...
const newHandlers = `async function getPackages(event, tenantId) {
    const pool = getPool();
    const { rows } = await pool.query(
        \`SELECT * FROM "Package"
         WHERE "tenantId" = $1
...
if (content.includes(oldHandlers)) {
    ...
    console.log('✓ Replaced package handlers successfully');
} else {
    console.log('✗ Could not find old handlers to replace');
}

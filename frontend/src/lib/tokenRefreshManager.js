...
async function performRefresh() {
  ...
  isRefreshing = true;:', refreshToken.substring(0, 20) + '...');
...
export function scheduleTokenRefresh(accessToken, onRefreshSuccess, onRefreshFailure) {
  ...
  const expiresAt = getTokenExpiration(accessToken);.toLocaleString() : 'unknown',
  ...
  const refreshTime = new Date(Date.now() + refreshIn);} min. Will refresh at ${refreshTime.toLocaleTimeString()} (in ${Math.round(refreshIn / 1000 / 60)} min)`);

...
const EmptyState = ({ hasFilters, onClearFilters, onAddOwner }) => (
  <div className="flex-1 flex flex-col items-center justify-center py-24 rounded-2xl border backdrop-blur-[16px] bg-[var(--bb-glass-bg)] border-[var(--bb-glass-border)] shadow-[0_8px_32px_rgba(0,0,0,0.08),_inset_0_0_0_1px_rgba(255,255,255,0.1)] dark:shadow-[0_8px_32px_rgba(0,0,0,0.3),_inset_0_0_0_1px_rgba(255,255,255,0.05)]">
    ...
    <div className="relative mb-8">
      <div className="absolute inset-0 rounded-3xl blur-2xl opacity-40 bg-gradient-to-br from-blue-500 to-cyan-500" aria-hidden="true" />
      <div className="relative flex h-24 w-24 items-center justify-center rounded-3xl bg-gradient-to-br from-blue-500 to-cyan-500 shadow-xl">
        <Users className="h-12 w-12 text-white" strokeWidth={1.5} />
      </div>
    </div>
    <h3 className="text-2xl font-bold text-[color:var(--bb-color-text-primary)] mb-3">{hasFilters ? 'No owners match your filters' : 'No owners yet'}</h3>
    <p className="text-base text-[color:var(--bb-color-text-muted)] mb-8 max-w-md text-center">{hasFilters ? 'Try adjusting your search or filters to find what you\'re looking for' : 'Get started by adding your first pet owner to the system'}</p>
    <div className="flex gap-3">
      {hasFilters && <Button variant="outline" size="lg" onClick={onClearFilters}>Clear filters</Button>}
      <Button size="lg" onClick={onAddOwner} className="shadow-lg hover:shadow-xl transition-shadow"><Plus className="h-4 w-4 mr-2" />Add Owner</Button>
    </div>
  </div>
);

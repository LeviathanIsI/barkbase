    ...
    <Card className={`p-6 hover:shadow-lg transition-all duration-200 ${isSelected ? 'ring-2 ring-blue-500' : ''}`}>
      ...
      {booking.customerHistory && (
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 mb-4">
          <h4 className="font-medium text-blue-900 mb-2">📊 CUSTOMER HISTORY</h4>
          <div className="grid grid-cols-2 gap-4 text-sm">
            <div>
              <span className="text-blue-700">Total visits:</span>
              <div className="font-semibold text-blue-900">{booking.customerHistory.totalVisits || 0}</div>
            </div>
            <div>
            ...
            <div>
              <span className="text-blue-700">Lifetime value:</span>
              <div className="font-semibold text-blue-900">
      ...
      {booking.status === 'checked_in' && booking.activities && Array.isArray(booking.activities) && booking.activities.length > 0 && (
        <div className="bg-purple-50 border border-purple-200 rounded-lg p-3 mb-4">
          <h4 className="font-medium text-purple-900 mb-2">📋 TODAY'S ACTIVITIES</h4>
          <div className="space-y-2">
            {booking.activities.map((activity, index) => (
              <div key={index} className="flex items-center gap-2 text-sm">
                <CheckCircle className="w-4 h-4 text-purple-600" />
                <span className="text-purple-800">{activity.time} - {activity.activity}</span>
              </div>
            ))}
          </div>
          <p className="text-xs text-purple-700 mt-2">📸 3 photos captured • View Photos</p>
        </div>
      )}

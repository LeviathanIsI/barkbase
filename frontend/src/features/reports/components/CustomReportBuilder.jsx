...
// Verified against: backend/database/migrations/2025-12-22-seed-system-properties.sql
const SAMPLE_REPORTS = [
  {
    id: 'bookings-by-status',
    name: 'Bookings by Status',
    description: 'See booking status distribution',
    icon: CalendarDays,
    color: '#10B981',
    dataSource: 'bookings',
    chartType: 'bar',
  ...
  },
  {
    ...
    description: 'Track booking trends by check-in date',
    icon: Activity,
    color: '#8B5CF6',
    dataSource: 'bookings',
    chartType: 'line',
    config: {
      xAxis: { key: 'check_in', label: 'Check-In' },
      yAxis: { key: 'count', label: 'Count of Bookings' },
    },
  },
  {
  ...
  },
  {
    id: 'pets-by-breed',
    name: 'Pets by Breed',
    description: 'See the most popular breeds',
    icon: PawPrint,
    color: '#3B82F6',
    dataSource: 'pets',
    chartType: 'column',
    config: {
      xAxis: { key: 'breed', label: 'Breed' },
      yAxis: { key: 'count', label: 'Count of Pets' },
    },
  },
  {
    id: 'services-by-category',
    name: 'Services by Category',
    description: 'Services breakdown by category',
    icon: Wrench,
    color: '#EC4899',
    dataSource: 'services',
    chartType: 'donut',
    config: {
      breakDownBy: { key: 'category', label: 'Category' },
      values: { key: 'count', label: 'Count of Services' },
    },
  },
  {
    ...
    chartType: 'donut',
    config: {
      breakDownBy: { key: 'method', label: 'Payment Method' },
                ...
                {hoveredChartType === 'table' ? (
                  ...
                  <ResponsiveContainer width="100%" height="100%">
                    {hoveredChartType === 'bar' ? (
                      ...
                      <PieChart>
                        <Pie data={[{ name: 'Occupied Kennels', value: 73 }, { name: 'Available Kennels', value: 27 }]} dataKey="value" nameKey="name" cx="50%" cy="60%" innerRadius={80} outerRadius={140} startAngle={180} endAngle={0}>
                  ...
                  {hoveredChartType === 'gauge' && (
                    <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none" style={{ paddingTop: '5%' }}>
                      <span className="text-4xl font-semibold text-text">73%</span>
                      <span className="text-sm text-muted mt-1">Kennel Occupancy</span>
                    </div>
                  )}
                  </div>
                )}

...
const DiscountApplyConfig = ({ node, onUpdate }) => {
  ...
  const handleSave = () => {
    ...
    const validationErrors = validateActionConfig('discount.apply', newConfig);
  ...
  return (
    <div className="space-y-4">
      <div>
        <h3 className="text-sm font-semibold text-text mb-3">Apply Discount Configuration</h3>
        ...
        <div className="space-y-3">
          ...
          <div>
            <label className="text-xs font-medium text-text mb-1 block">Discount Amount *</label>
            <input
              type="number"
              value={amount}
          ...
          <div>
            <label className="text-xs font-medium text-text mb-1 block">Discount Code (optional)</label>
            <input
              type="text"
              value={code}
              onChange={(e) => setCode(e.target.value)}
              className="w-full px-3 py-2 text-sm border border-border rounded bg-background text-text"
              placeholder="e.g., REFERRAL10"
            />
          </div>
...
export default DiscountApplyConfig;

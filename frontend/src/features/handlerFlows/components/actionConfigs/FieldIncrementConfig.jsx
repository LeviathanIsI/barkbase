import { useState } from 'react';
import Button from '@/components/ui/Button';
import { validateActionConfig } from '../../utils/validateDefinition';

const FieldIncrementConfig = ({ node, onUpdate }) => {
  const config = node?.data?.config || {};

  const [object, setObject] = useState(config?.object || 'owner');
  const [field, setField] = useState(config?.field || '');
  const [delta, setDelta] = useState(config?.delta || 1);
  const [errors, setErrors] = useState([]);

  const handleSave = () => {
    const newConfig = {
      object,
      field,
      delta: parseFloat(delta),
    };

    const validationErrors = validateActionConfig('field.increment', newConfig);
    if (validationErrors.length > 0) {
      setErrors(validationErrors);
      return;
    }

    setErrors([]);
    onUpdate(node.id, {
      actionType: 'field.increment',
      config: newConfig,
      label: node.data.label || 'Increment Field',
    });
  };

  return (
    <div className="space-y-4">
      <div>
        <h3 className="text-sm font-semibold text-text mb-3">Increment Field Configuration</h3>

        <div className="space-y-3">
          <div>
            <label className="text-xs font-medium text-text mb-1 block">Object *</label>
            <select
              value={object}
              onChange={(e) => setObject(e.target.value)}
              className="w-full px-3 py-2 text-sm border border-border rounded bg-background text-text"
            >
              <option value="owner">Owner</option>
              <option value="pet">Pet</option>
              <option value="reservation">Reservation</option>
              <option value="invoice">Invoice</option>
              <option value="tenant">Tenant</option>
              <option value="custom">Custom</option>
            </select>
          </div>

          <div>
            <label className="text-xs font-medium text-text mb-1 block">Field Name *</label>
            <input
              type="text"
              value={field}
              onChange={(e) => setField(e.target.value)}
              className="w-full px-3 py-2 text-sm border border-border rounded bg-background text-text"
              placeholder="e.g., totalVisits"
            />
          </div>

          <div>
            <label className="text-xs font-medium text-text mb-1 block">Delta (increment by) *</label>
            <input
              type="number"
              value={delta}
              onChange={(e) => setDelta(e.target.value)}
              className="w-full px-3 py-2 text-sm border border-border rounded bg-background text-text"
              placeholder="1"
              step="any"
            />
            <div className="text-xs text-muted mt-1">Use negative values to decrement</div>
          </div>
        </div>

        {errors.length > 0 && (
          <div className="mt-3 p-3 bg-red-500/10 border border-red-500/30 rounded">
            <div className="text-xs font-medium text-red-400 mb-1">Validation Errors:</div>
            <ul className="text-xs text-red-300 list-disc list-inside">
              {errors.map((err, i) => <li key={i}>{err}</li>)}
            </ul>
          </div>
        )}
      </div>

      <div className="pt-2 border-t border-border">
        <Button onClick={handleSave} variant="primary" size="sm" className="w-full">
          Save Configuration
        </Button>
      </div>
    </div>
  );
};

export default FieldIncrementConfig;
